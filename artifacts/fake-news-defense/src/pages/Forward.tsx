...
import {
  MessageCircle, Send, Loader2, ShieldCheck, AlertTriangle,
  XOctagon, HelpCircle, Share2, Copy, Check, Sparkles,
} from "lucide-react";
...
function stripForwardingNoise(text: string): string {
  return text
...
function verdictMeta(verdict: string) {
  ...
  if (v === "real") return { Icon: ShieldCheck, color: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-50 dark:bg-emerald-950/30 border-emerald-200 dark:border-emerald-800", label: "REAL", emoji: "✅" };
  if (v === "fake") return { Icon: XOctagon, color: "text-red-600 dark:text-red-400", bg: "bg-red-50 dark:bg-red-950/30 border-red-200 dark:border-red-800", label: "FAKE", emoji: "❌" };
  if (v === "misleading") return { Icon: AlertTriangle, color: "text-amber-600 dark:text-amber-400", bg: "bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800", label: "MISLEADING", emoji: "⚠️" };
...
function detectForwardingSignals(text: string): string[] {
  const signals: string[] = [];
  if (/forwarded/i.test(text)) signals.push("Contains 'Forwarded' label");
  if (/urgent|breaking|important/i.test(text)) signals.push("Uses urgency language (URGENT / BREAKING)");
  if (/share|forward|send to everyone/i.test(text)) signals.push("Pressures you to share immediately");
  if (/before (it'?s?|they) (deleted?|removed?)/i.test(text)) signals.push("Claims it will be deleted — manipulation tactic");
  if (/government|they don't want you to know|mainstream media won't tell/i.test(text)) signals.push("Anti-establishment framing");
  if (/doctor|scientist|expert|nasa|who|cdc/i.test(text)) signals.push("Claims authority without verifiable source");
  if (/cure|guaranteed|100%|proven/i.test(text)) signals.push("Absolute language — real science is rarely 100%");
  return signals;
}
    ...
    <div className="container mx-auto max-w-2xl px-4 py-10">
      ...
      <Card className="border-border/60 shadow-lg mb-5">
        <CardContent className="p-4">
          ...
          <Textarea
            value={message}
            onChange={(e) => { setMessage(e.target.value); if (result) reset(); }}
            placeholder="Paste the forwarded message here... e.g. 'URGENT: Scientists confirm that...' or any viral news message you received"
            className="resize-none border-0 shadow-none focus-visible:ring-0 min-h-[120px] bg-transparent text-sm p-0"
            rows={4}
          />

          {/* Forwarding signals live detector */}
          <AnimatePresence>
            {message.length > 20 && signals.length > 0 && !result && (
              <motion.div
          ...
          <div className="flex items-center justify-between mt-3 pt-3 border-t border-border/40">
            ...
            <Button
              onClick={analyze}
              disabled={isPending || !cleaned.trim()}
              className="satya-gradient border-0 rounded-xl gap-2 shadow-md"
            >
          ...
          <div className="space-y-2">
            {EXAMPLE_MESSAGES.map((ex, i) => (
              <button
                ...
                className="w-full text-left text-xs p-3 rounded-xl bg-muted/40 border border-border hover:bg-primary/5 hover:border-primary/30 transition-all text-muted-foreground hover:text-foreground line-clamp-2"
              >
                {ex}
              </button>
            ))}
                ...
                <div className="h-2 rounded-full bg-black/10 dark:bg-white/10 overflow-hidden">
                  <motion.div
                    ...
                    className={cn("h-full rounded-full",
                      meta.label === "REAL" ? "bg-emerald-500" : meta.label === "FAKE" ? "bg-red-500" : meta.label === "MISLEADING" ? "bg-amber-500" : "bg-blue-400"
                    )}
                  />
                </div>
              )}
              ...
              <Card className="border-border/60 mb-4">
                <CardContent className="p-5">
                  ...
                  <ul className="space-y-1.5">
                    {((result as any).keyPoints as string[]).map((point: string, i: number) => (
