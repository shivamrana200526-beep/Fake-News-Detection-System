...
import {
  Brain, FileText, Globe, Image as ImageIcon, Link2, Loader2, AlertTriangle,
...
export default function Detect() {
  const [activeTab, setActiveTab] = useState<"text" | "url" | "headline" | "image">("text");
  ...
  // Voice input
  const { isListening, isSupported: voiceSupported, start: startListening, stop: stopListening } = useVoiceInput(
    (transcript) => setContent((prev) => prev ? prev + " " + transcript : transcript)
  );
  ...
  const renderVerdictBadge = (prediction: string) => {
    switch (prediction.toLowerCase()) {
      case "real":
        return (
          <Badge className="bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-800 text-base py-1.5 px-5 hover:bg-emerald-100">
            <ShieldCheck className="mr-2 h-5 w-5" /> REAL
          </Badge>
        );
      case "fake":
        return (
          <Badge className="bg-red-100 text-red-700 dark:bg-red-950/40 dark:text-red-400 border border-red-300 dark:border-red-800 text-base py-1.5 px-5 hover:bg-red-100">
            <XOctagon className="mr-2 h-5 w-5" /> FAKE
          </Badge>
        );
      default:
        return (
          <Badge className="bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-400 border border-amber-300 dark:border-amber-800 text-base py-1.5 px-5 hover:bg-amber-100">
  ...
  return (
    <div className="container mx-auto max-w-6xl px-4 flex flex-col gap-8">
      {/* Header */}
        ...
        <div className="lg:col-span-5 space-y-4">
          <Card className="shadow-lg border-border/60">
            ...
            <CardContent className="pt-6">
              <Tabs value={activeTab} onValueChange={(v) => { setActiveTab(v as any); setContent(""); }} className="w-full">
                ...
                <div className="min-h-[200px]">
                  ...
                  <TabsContent value="headline" className="mt-0">
                    <Textarea
                      placeholder="Enter the news headline or claim..."
                      className="min-h-[140px] resize-y text-base p-4"
                      value={content}
                      onChange={(e) => setContent(e.target.value)}
                    />
                  </TabsContent>
              ...
              {activeTab !== "image" && (
                <div className="mt-4 flex gap-2">
                  <Button
                    ...
                    className="flex-1 gap-2 text-xs rounded-lg"
                  >
                  ...
                  {voiceSupported && (
                    <Button
                      ...
                      className={cn("gap-2 text-xs rounded-lg", isListening && "animate-pulse")}
                    >
                      {isListening ? (
              ...
              {isListening && (
                <motion.div
                  ...
                  className="mt-3 flex items-center gap-2 text-sm text-destructive font-medium px-3 py-2 bg-destructive/10 rounded-lg border border-destructive/20"
                >
              ...
              <div className="mt-5 pt-5 border-t border-border">
                <Button
                  className="w-full h-13 text-base rounded-xl shadow-md hover:shadow-lg transition-all satya-gradient border-0"
                ...
                {error && (
                  <p className="text-destructive text-sm mt-3 flex items-center justify-center gap-2">
                    ...
                    {(error as any).message || "Failed to analyze. Please try again."}
                  </p>
                )}
              </div>
            </CardContent>
          ...
          <AnimatePresence mode="wait">
            {!result && !isPending && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="h-full min-h-[480px] flex flex-col items-center justify-center border-2 border-dashed border-border rounded-2xl p-12 text-center text-muted-foreground bg-muted/10"
              >
            ...
            {isPending && (
              <motion.div
                ...
                className="h-full min-h-[480px] flex flex-col items-center justify-center border border-border rounded-2xl p-12 text-center bg-card shadow-lg"
              >
              ...
              >
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {(result as any).scrapedArticle.title && (
                            <p className="font-semibold text-foreground text-sm leading-snug mb-1 truncate">{(result as any).scrapedArticle.title}</p>
                          )}
                          <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
                            {(result as any).scrapedArticle.author && (
                              <span className="flex items-center gap-1"><User className="h-3 w-3" />{(result as any).scrapedArticle.author}</span>
                            )}
                            {(result as any).scrapedArticle.publishDate && (
                              <span className="flex items-center gap-1"><Calendar className="h-3 w-3" />{new Date((result as any).scrapedArticle.publishDate).toLocaleDateString()}</span>
                            )}
                            {(result as any).scrapedArticle.wordCount > 0 && (
                              <span className="flex items-center gap-1"><Eye className="h-3 w-3" />{(result as any).scrapedArticle.wordCount} words read</span>
                            )}
                          </div>
                        </div>
                      </div>
                ...
                <Card className="border-t-4 shadow-xl overflow-hidden" style={{ borderTopColor: verdictBorderColor }}>
                  <CardContent className="p-6 md:p-8">
                    ...
                    {/* Manipulation Score */}
                    {(result as any).manipulationScore !== undefined && (
                      <div className="mb-5 p-4 rounded-xl border border-border/50 bg-background">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className={cn(
                            "text-sm font-black",
                            (result as any).manipulationScore >= 60 ? "text-red-600 dark:text-red-400" :
                            (result as any).manipulationScore >= 35 ? "text-amber-600 dark:text-amber-400" :
                            "text-emerald-600 dark:text-emerald-400"
                          )}>
                            {(result as any).manipulationScore}/100
                          </span>
                        </div>
                        <div className="h-3 rounded-full bg-muted overflow-hidden">
                          <motion.div
                            initial={{ width: 0 }}
                            animate={{ width: `${(result as any).manipulationScore}%` }}
                            transition={{ duration: 0.8, ease: "easeOut" }}
                            className={cn("h-full rounded-full",
                              (result as any).manipulationScore >= 60 ? "bg-red-500" :
                              (result as any).manipulationScore >= 35 ? "bg-amber-500" :
                              "bg-emerald-500"
                            )}
                          />
                        </div>
                        <p className="text-xs text-muted-foreground mt-1.5">
                          {(result as any).manipulationScore >= 60 ? "⚠️ Heavy emotional manipulation detected" :
                           (result as any).manipulationScore >= 35 ? "🔶 Moderate manipulation tactics present" :
                    ...
                    {/* Emotional Tactics */}
                    {(result as any).emotionalTactics?.length > 0 && (
                      <div className="mb-5">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {(result as any).emotionalTactics.map((tactic: string, i: number) => (
                            <span key={i} className="text-xs px-2.5 py-1 rounded-full bg-amber-100 dark:bg-amber-950/30 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-900 font-medium">
                              ⚠ {tactic}
                            </span>
                          ))}
                        </div>
                    ...
                    {/* Logical Fallacies */}
                    {(result as any).logicalFallacies?.length > 0 && (
                      <div className="mb-5">
                        <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2 block">
                          🧠 Logical Fallacies Detected
                        </span>
                        <div className="flex flex-wrap gap-1.5">
                          {(result as any).logicalFallacies.map((fallacy: string, i: number) => (
                            <span key={i} className="text-xs px-2.5 py-1 rounded-full bg-violet-100 dark:bg-violet-950/30 text-violet-700 dark:text-violet-400 border border-violet-200 dark:border-violet-900 font-medium">
                              {fallacy}
                            </span>
                          ))}
                        </div>
                    ...
                    {/* Missing Context */}
                    {(result as any).missingContext && (
                      <div className="mb-5 p-3 rounded-xl bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-900">
                        <p className="text-xs font-bold text-blue-700 dark:text-blue-400 mb-1">📌 Missing Context</p>
                        <p className="text-xs text-blue-600 dark:text-blue-300 leading-relaxed">{(result as any).missingContext}</p>
                      </div>
                    )}
                ...
                {/* Fact Breakdown */}
                {result.factBreakdown && (result.factBreakdown as any[]).length > 0 && (
                  <Card className="shadow-md">
                    ...
                    <CardContent className="pt-4 space-y-2">
                      {(result.factBreakdown as any[]).map((fb: any, i: number) => (
                        <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-muted/30 border border-border/40">
                          {fb.status === "real" ? (
                            <ShieldCheck className="h-4 w-4 text-emerald-500 mt-0.5 shrink-0" />
                          ) : fb.status === "fake" ? (
                            <XOctagon className="h-4 w-4 text-red-500 mt-0.5 shrink-0" />
                          ) : fb.status === "misleading" ? (
                          ...
                          <span className={cn(
                            ...
                            fb.status === "real" ? "bg-emerald-50 dark:bg-emerald-950/20 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-900" :
                ...
                {result.geminiVerification && (
                  <Card className="shadow-md border-blue-200/70 dark:border-blue-900/70 bg-blue-50/20 dark:bg-blue-950/10">
                    <CardHeader className="pb-3 border-b border-border/40">
                    ...
                    <CardContent className="pt-4 space-y-3">
                      ...
                      {result.geminiVerification.factPoints && result.geminiVerification.factPoints.length > 0 && (
                        <ul className="space-y-2">
                          {result.geminiVerification.factPoints.map((fp, i) => (
                            <li key={i} className="flex items-start gap-2 text-sm bg-background p-3 rounded-lg border border-border/40">
                              {fp.status === "verified" ? (
                                <ShieldCheck className="h-4 w-4 text-emerald-500 mt-0.5 shrink-0" />
                              ) : fp.status === "false" ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Card className="shadow-sm border-border/50">
                    <CardContent className="p-5">
                      ...
                      {result.detectedSource && (result.detectedSource as any).name ? (
                        <a
                          href={(result.detectedSource as any).url}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-2 text-primary font-medium hover:underline p-2 rounded-lg hover:bg-muted transition-colors text-sm"
                        >
                          {(result.detectedSource as any).name}
                          <ExternalLink className="h-3.5 w-3.5" />
                        </a>
                      ) : (
                        <span className="text-sm text-muted-foreground">Source unknown.</span>
                      )}
                      {(result as any).authorCredibility && (
                        <p className="text-xs text-muted-foreground mt-2 pt-2 border-t border-border/40 leading-relaxed">
                          {(result as any).authorCredibility}
                        </p>
                      )}
                    </CardContent>
                    ...
                    <CardContent className="p-5">
                      ...
                      <div className="flex gap-2 flex-wrap">
                        {["04", "09", "10", "16"].map((num) => (
                          <img
                            key={num}
                            src={`https://sdgs.un.org/sites/default/files/goals/E_SDG_Icons-${num}.jpg`}
                            alt={`SDG ${num}`}
                            className="h-10 w-10 rounded-lg shadow-sm hover:scale-110 transition-transform cursor-help"
                          />
                        ))}
                      </div>
                    </CardContent>
                ...
                <Button
                  ...
                  onClick={() => { setContent(""); window.scrollTo({ top: 0, behavior: "smooth" }); }}
                >
