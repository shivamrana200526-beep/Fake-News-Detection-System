...
import {
  ShieldCheck, ShieldAlert, ShieldX, Globe, Loader2,
  TrendingUp, AlertTriangle, CheckCircle2, XCircle, HelpCircle,
  Info, Sparkles,
} from "lucide-react";
...
function trustColor(score: number) {
  if (score >= 75) return { text: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-500", ring: "ring-emerald-200 dark:ring-emerald-900", card: "bg-emerald-50 dark:bg-emerald-950/30 border-emerald-200 dark:border-emerald-800" };
  if (score >= 50) return { text: "text-amber-600 dark:text-amber-400", bg: "bg-amber-500", ring: "ring-amber-200 dark:ring-amber-900", card: "bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800" };
...
function biasColor(bias: string) {
  ...
  if (b.includes("far left") || b.includes("extreme left")) return "text-blue-700 dark:text-blue-300 bg-blue-100 dark:bg-blue-950/30";
  if (b.includes("center-left") || b.includes("left")) return "text-blue-500 bg-blue-50 dark:bg-blue-950/20";
  if (b.includes("center")) return "text-gray-600 dark:text-gray-300 bg-gray-100 dark:bg-gray-800";
  if (b.includes("center-right") || b.includes("right")) return "text-red-500 bg-red-50 dark:bg-red-950/20";
  if (b.includes("far right") || b.includes("extreme right")) return "text-red-700 dark:text-red-300 bg-red-100 dark:bg-red-950/30";
  return "text-muted-foreground bg-muted";
}

function ratingIcon(rating: string) {
  const r = rating?.toLowerCase() || "";
  if (r.includes("highly reliable")) return <CheckCircle2 className="h-4 w-4 text-emerald-500" />;
  if (r.includes("generally reliable")) return <CheckCircle2 className="h-4 w-4 text-green-400" />;
  if (r.includes("mixed")) return <AlertTriangle className="h-4 w-4 text-amber-500" />;
  if (r.includes("unreliable")) return <XCircle className="h-4 w-4 text-red-500" />;
  if (r.includes("satire")) return <Info className="h-4 w-4 text-blue-400" />;
      ...
      <div className={cn("relative w-36 h-18 flex items-end justify-center")}>
        <svg width="144" height="80" viewBox="0 0 144 80">
          ...
          <path
            ...
            className={cn(score >= 75 ? "stroke-emerald-500" : score >= 50 ? "stroke-amber-500" : "stroke-red-500")}
          />
          <circle
            cx={72 + 60 * Math.cos(((angle - 90) * Math.PI) / 180)}
            cy={72 + 60 * Math.sin(((angle - 90) * Math.PI) / 180)}
            r="7"
            className={cn("fill-background stroke-2", score >= 75 ? "stroke-emerald-500" : score >= 50 ? "stroke-amber-500" : "stroke-red-500")}
          />
        </svg>
...
export default function Credibility() {
  ...
  return (
    <div className="container mx-auto max-w-3xl px-4 py-10">
        ...
        <CardContent className="p-4">
          <div className="flex gap-3">
            <div className="relative flex-1">
              ...
              <Input
                ...
                placeholder="e.g. bbc.com, reuters.com, infowars.com"
            ...
            <Button
              ...
              className="h-11 px-6 satya-gradient border-0 shadow-md"
            >
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            ...
            {EXAMPLE_SOURCES.map((s) => (
              <button
                key={s}
                onClick={() => { setSource(s); check(s); }}
                className="text-xs px-2.5 py-1 rounded-full border border-border bg-muted/50 hover:bg-primary/5 hover:border-primary/30 hover:text-primary transition-all text-muted-foreground"
              >
                {s}
              </button>
            ))}
      ...
      {/* Error */}
      {error && (
        <div className="p-4 rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 text-sm mb-6">
          {error}
        </div>
      )}
                  ...
                  <div className="flex-1 text-center sm:text-left">
                    <div className="flex items-center justify-center sm:justify-start gap-2 mb-1">
                      {result.trustScore >= 75 ? (
                        ...
                        <ShieldAlert className={cn("h-5 w-5", colors.text)} />
                      ) : (
                        <ShieldX className={cn("h-5 w-5", colors.text)} />
                      )}
