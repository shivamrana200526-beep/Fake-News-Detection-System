...
import {
  History as HistoryIcon, ShieldCheck, AlertTriangle, XOctagon,
  HelpCircle, Trash2, Download, Search, Clock, BarChart3,
} from "lucide-react";
...
function verdictMeta(verdict: string) {
  ...
  if (v === "real") return { Icon: ShieldCheck, color: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-50 dark:bg-emerald-950/30 border-emerald-200 dark:border-emerald-800", dot: "bg-emerald-500", label: "Real", emoji: "✅" };
  if (v === "fake") return { Icon: XOctagon, color: "text-red-600 dark:text-red-400", bg: "bg-red-50 dark:bg-red-950/30 border-red-200 dark:border-red-800", dot: "bg-red-500", label: "Fake", emoji: "❌" };
  if (v === "misleading") return { Icon: AlertTriangle, color: "text-amber-600 dark:text-amber-400", bg: "bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800", dot: "bg-amber-500", label: "Misleading", emoji: "⚠️" };
  ...
  return (
    <div className="container mx-auto max-w-3xl px-4 py-10">
      ...
      {entries.length > 0 && (
        <div className="flex gap-3 mb-5 flex-wrap">
          ...
          <div className="flex gap-1.5">
            {["All", "Real", "Fake", "Misleading"].map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={cn(
                  "text-xs px-3 py-1.5 rounded-full border font-medium transition-all",
                  filter === f ? "satya-gradient text-white border-transparent" : "border-border bg-muted/40 text-muted-foreground hover:bg-muted"
                )}
              >
                {f}
              </button>
      ...
      {entries.length > 0 && filtered.length === 0 && (
        <div className="text-center py-12 text-muted-foreground">No results match your search.</div>
      )}
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {entry.explanation && (
                          <p className="text-xs text-muted-foreground mt-1 line-clamp-1">{entry.explanation}</p>
                        )}
                      </div>
                      <button
                        onClick={() => removeOne(entry.id)}
                        className="opacity-0 group-hover:opacity-100 transition-opacity p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/20 text-muted-foreground hover:text-red-500 shrink-0"
                      >
