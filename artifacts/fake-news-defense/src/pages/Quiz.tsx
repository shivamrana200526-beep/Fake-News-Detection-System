...
import {
  Brain, ShieldCheck, AlertTriangle, XOctagon, Loader2,
  ChevronRight, Star, Trophy, RefreshCw, HelpCircle,
} from "lucide-react";
...
type UserAnswer = "Real" | "Fake" | "Misleading";

const VERDICT_OPTIONS: { label: string; value: UserAnswer; icon: React.FC<any>; style: string; active: string }[] = [
  {
    label: "Real",
    value: "Real",
    icon: ShieldCheck,
...
function difficultyColor(d: string) {
  if (d === "Easy") return "text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/20 border-emerald-200";
  if (d === "Hard") return "text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/20 border-red-200";
  return "text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/20 border-amber-200";
}

function resultStyle(correct: boolean) {
  return correct
...
function ScoreScreen({ score, total, totalPoints, maxPoints, onRetry }: {
  score: number; total: number; totalPoints: number; maxPoints: number; onRetry: () => void;
}) {
      ...
      <div className="max-w-xs mx-auto mb-8">
        <div className="h-3 rounded-full bg-muted overflow-hidden">
          <motion.div
            ...
            className="h-full rounded-full satya-gradient"
          />
        </div>
                    ...
                    <AnimatePresence>
                      {showHint && (
                        <motion.p
                          ...
                          className="text-xs text-amber-600 dark:text-amber-400 mt-1.5 bg-amber-50 dark:bg-amber-950/20 px-3 py-2 rounded-lg border border-amber-200 dark:border-amber-900"
                        >
                          💡 {q.clue}
                        </motion.p>
                      )}
                    ...
                    {VERDICT_OPTIONS.map((opt) => {
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            "flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all duration-200 font-semibold text-sm",
                            confirmed
                              ? isCorrect
                  ...
                  <AnimatePresence>
                    {confirmed && (
                      <motion.div
                        ...
                        animate={{ opacity: 1, height: "auto" }}
                        className={cn(
                          "p-4 rounded-xl border mb-4 overflow-hidden",
                          resultStyle(selected === q.verdict).bg
                        )}
                      >
                  ...
                  <div className="flex gap-3">
                    {!confirmed ? (
                      <Button
                        onClick={confirm}
                        disabled={!selected}
                        className="flex-1 satya-gradient border-0 rounded-xl gap-2"
                      >
                        Submit Answer
                      </Button>
                    ) : (
          ...
          {answers.length > 0 && (
            <div className="flex gap-1.5 mt-4 justify-center flex-wrap">
              {answers.map((a, i) => (
                <div
                  key={i}
                  className={cn("w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white shadow-sm",
              ...
              {Array.from({ length: questions.length - answers.length }).map((_, i) => (
                <div key={`empty-${i}`} className="w-8 h-8 rounded-full border-2 border-dashed border-border/40" />
              ))}
            </div>
          )}
        </>
