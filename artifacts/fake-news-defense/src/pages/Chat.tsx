...
import {
  Send, Loader2, Bot, User, Sparkles, Mic, MicOff,
  RefreshCw, Copy, Check, ShieldCheck, AlertTriangle,
  XOctagon, HelpCircle, Zap, Search,
} from "lucide-react";
...
interface FactPoint {
  point: string;
  status: "verified" | "false" | "disputed" | "unknown";
}
...
interface AIVerdict {
  ...
  confidence: number;
  summary: string;
  factPoints: FactPoint[];
  redFlags?: string[];
  contextNote?: string;
  manipulationTactic?: string | null;
  logicalIssue?: string | null;
}
...
interface VerdictData {
  ...
  claim: string;
  claimCount?: number;
  claimBreakdowns?: ClaimBreakdown[];
  gpt: AIVerdict | null;
...
function factStatusColor(status: string) {
  ...
  if (status === "disputed") return "text-amber-500 dark:text-amber-400";
  return "text-muted-foreground";
}

function factStatusIcon(status: string) {
      ...
      <div className={cn("rounded-2xl border p-4 shadow-md", meta.bg)}>
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            ...
            className="text-xs text-muted-foreground hover:text-foreground border border-border/60 px-2.5 py-1 rounded-lg transition-colors bg-background/60"
          >
        ...
        {data.claimBreakdowns && data.claimBreakdowns.length > 1 && (
          <div className="mb-3 space-y-1">
            ...
            {data.claimBreakdowns.map((cb, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-2 bg-background/60 border border-border/40 rounded-xl px-2.5 py-1.5 text-xs">
              ...
              <div className="border-t border-border/40 pt-3 mt-1 space-y-4">
                ...
                {data.gpt && (
                  <div>
                    ...
                    {data.gpt.factPoints.length > 0 && (
                      <div className="space-y-1">
                        {data.gpt.factPoints.map((fp, i) => (
                    ...
                    {data.gpt.redFlags && data.gpt.redFlags.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-1">
                        {data.gpt.redFlags.map((flag, i) => (
                          <span key={i} className="text-[10px] px-2 py-0.5 rounded-full bg-red-100 dark:bg-red-950/30 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-900">
                            ⚠ {flag}
                          </span>
                        ))}
                      </div>
                    )}
                    {data.gpt.manipulationTactic && (
                      <p className="mt-2 text-[10px] px-2.5 py-1.5 rounded-lg bg-amber-50 dark:bg-amber-950/30 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-900">
                ...
                {data.gemini && (
                  <div>
                    ...
                    {data.gemini.factPoints.length > 0 && (
                      <div className="space-y-1">
                        {data.gemini.factPoints.map((fp, i) => (
                          <div key={i} className="flex items-start gap-2 text-xs">
                            <span className="shrink-0 mt-0.5">{factStatusIcon(fp.status)}</span>
                            <span className={cn("leading-relaxed", factStatusColor(fp.status))}>{fp.point}</span>
                          </div>
                    ...
                    {data.gemini.contextNote && (
                      <p className="mt-2 text-xs text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/30 px-3 py-2 rounded-lg border border-blue-200 dark:border-blue-900">
                        📌 {data.gemini.contextNote}
                      </p>
                    )}
                    {data.gemini.logicalIssue && (
                      <p className="mt-2 text-[10px] px-2.5 py-1.5 rounded-lg bg-violet-50 dark:bg-violet-950/30 text-violet-700 dark:text-violet-400 border border-violet-200 dark:border-violet-900">
...
function formatContent(content: string) {
  return content.split("\n").map((line, i) => {
    ...
    if (line.startsWith("## ")) return <h3 key={i} className="font-bold text-base mt-3 mb-1" dangerouslySetInnerHTML={{ __html: line.replace("## ", "") }} />;
    if (line.startsWith("# ")) return <h2 key={i} className="font-bold text-lg mt-3 mb-1" dangerouslySetInnerHTML={{ __html: line.replace("# ", "") }} />;
  ...
  return (
    <div className={cn("flex gap-3", isUser ? "flex-row-reverse" : "flex-row")}>
      ...
      <div className={cn(
        ...
        isUser ? "satya-gradient text-white" : "bg-muted border border-border text-primary"
      )}>
      ...
      <div className={cn("max-w-[80%] flex flex-col gap-2", isUser ? "items-end" : "items-start")}>
        ...
        {/* Text bubble */}
        {(msg.isAnalyzing || msg.content || msg.isStreaming) && (
          <div className={cn("relative group w-full")}>
            <div className={cn(
              "rounded-2xl px-5 py-4 shadow-sm text-sm space-y-1",
              isUser ? "satya-gradient text-white rounded-tr-sm" : "bg-card border border-border/60 text-foreground rounded-tl-sm"
            )}>
              {msg.isAnalyzing ? (
                <div className="space-y-2">
                  {msg.scrapedArticle && (
                    <div className="flex items-center gap-2 text-xs bg-primary/10 border border-primary/20 rounded-lg px-3 py-2 text-primary font-medium mb-2">
                      <span>📰</span>
                ...
                <div className="flex items-center gap-1.5 py-1">
                  {[0, 0.15, 0.3].map((d, i) => (
                    <motion.div key={i} animate={{ y: [0, -4, 0] }} transition={{ duration: 0.6, repeat: Infinity, delay: d }}
                      className={cn("w-2 h-2 rounded-full", isUser ? "bg-white/70" : "bg-primary/60")} />
                  ))}
                </div>
              ) : (
                <div className="space-y-1">
                  ...
                  {msg.isStreaming && (
                    <span className="inline-block w-0.5 h-4 bg-current animate-pulse ml-0.5 translate-y-0.5" />
                  )}
                </div>
              )}
            </div>
            ...
            {/* Copy */}
            {!msg.isStreaming && !msg.isAnalyzing && msg.content && (
              <button
                onClick={handleCopy}
                className={cn(
                  "absolute -bottom-6 opacity-0 group-hover:opacity-100 transition-opacity text-xs text-muted-foreground hover:text-foreground flex items-center gap-1",
...
export default function Chat() {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (data.type === "verdict") {
              setMessages((prev) =>
                prev.map((m) =>
                  m.id === assistantId ? { ...m, isAnalyzing: false, verdict: data.verdict } : m
                )
              );
            }
            ...
            if (data.type === "content") {
              ...
              setMessages((prev) =>
                prev.map((m) =>
                  m.id === assistantId ? { ...m, isAnalyzing: false, content: fullText } : m
                )
              );
            }
  ...
  return (
    <div className="container mx-auto max-w-4xl px-4 flex flex-col" style={{ height: "calc(100vh - 140px)" }}>
      {/* Header */}
            ...
            <div className="ml-12 flex flex-wrap gap-2">
              {QUICK_PROMPTS.map((prompt) => (
                <button
                  ...
                  className="text-xs text-left px-3 py-2 rounded-xl bg-muted/50 border border-border hover:bg-primary/5 hover:border-primary/30 hover:text-primary transition-all duration-200 text-muted-foreground"
                >
                  {prompt}
                </button>
              ))}
      ...
      <div className="flex items-center gap-2 pb-3 shrink-0 flex-wrap">
        {[
          ...
          <span key={ai.label} className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${ai.bg} ${ai.color}`}>
            {ai.label}
          </span>
        ))}
      ...
      <Card className="border-border/70 shadow-lg shrink-0">
        <CardContent className="p-3">
          {isListening && (
            <div className="mb-2 flex items-center gap-2 text-xs text-destructive font-medium px-3 py-2 bg-destructive/10 rounded-lg border border-destructive/20">
          ...
          <div className="flex items-end gap-3">
            <Textarea
              ...
              placeholder="Ask about any news claim, headline, or story... (Enter to send, Shift+Enter for new line)"
              className="resize-none border-0 shadow-none focus-visible:ring-0 text-sm min-h-[60px] max-h-[160px] bg-transparent"
              rows={2}
            />
            <div className="flex items-center gap-2 shrink-0 pb-1">
              {voiceSupported && (
                <Button
                  ...
                  className={cn("rounded-xl h-10 w-10 shrink-0", isListening && "animate-pulse")}
                >
              ...
              <Button
                ...
                className="rounded-xl h-10 w-10 p-0 satya-gradient border-0 shadow-md hover:shadow-lg transition-shadow shrink-0"
              >
