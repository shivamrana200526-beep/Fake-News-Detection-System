...
export default function Dashboard() {
  ...
  if (statsLoading || historyLoading) {
    return (
      <div className="container mx-auto max-w-7xl px-4 py-20 flex flex-col items-center justify-center min-h-[60vh]">
  ...
  return (
    <div className="container mx-auto max-w-7xl px-4 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        {stats.total > 0 && (
          <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-500/10 border border-emerald-200 dark:border-emerald-900 text-emerald-600 dark:text-emerald-400 text-sm font-semibold">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="shadow-md border-border/50">
          ...
          <CardContent className="pt-6 h-[300px]">
            {stats.total === 0 ? (
              <div className="h-full flex items-center justify-center text-muted-foreground text-sm">No data yet — start analyzing!</div>
            ) : (
        ...
        <Card className="shadow-md border-border/50">
          ...
          <CardContent className="pt-6 h-[300px]">
            {!stats.trendingTopics || stats.trendingTopics.length === 0 ? (
              <div className="h-full flex items-center justify-center text-muted-foreground text-sm">Trending keywords will appear after analyses.</div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={stats.trendingTopics} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                  ...
                  <Bar dataKey="count" radius={[0, 6, 6, 0]} barSize={20}>
                    {(stats.trendingTopics ?? []).map((_: any, index: number) => (
                      <Cell key={`cell-${index}`} fill={`hsl(${245 + index * 15} 58% 55%)`} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
