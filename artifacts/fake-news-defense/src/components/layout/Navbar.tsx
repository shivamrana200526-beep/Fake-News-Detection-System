...
import { ScanSearch, Menu, X, Sun, Moon, ChevronDown, MessageCircle, Globe, Brain, TrendingUp, History, MessageSquare } from "lucide-react";
...
export function Navbar() {
  ...
  return (
    ...
    >
          ...
          <nav className="hidden md:flex items-center gap-1">
            {mainLinks.map((link) => (
              ...
              >
                {link.label}
                {link.badge && (
                  <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full satya-gradient text-white leading-none">
                    {link.badge}
                  </span>
                )}
              </Link>
            ...
            <div className="relative" ref={toolsRef}>
              ...
              {toolsOpen && (
                <div className="absolute top-full left-0 mt-2 w-64 bg-background/95 backdrop-blur-xl border border-border/60 rounded-2xl shadow-xl p-2 z-50">
                  {toolLinks.map((tool) => {
      ...
      {/* Mobile Nav */}
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-background/95 backdrop-blur-xl border-b border-border shadow-xl py-4 px-4 flex flex-col gap-1 animate-in slide-in-from-top-2 max-h-[80vh] overflow-y-auto">
